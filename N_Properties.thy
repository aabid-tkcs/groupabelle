...
section\<open>N-Properties\<close>

text\<open>In this file, we define the properties N0, N1, and N2 [Ref: Lyndon and Schupp]. A key step
in the proof of Nielsen-Schreier, given in Lyndon and Schupp, is that given any
 subgroup of a freegroup, there exists a subset in which all the elements and their
 inverses satisfy the above properties, and the subset generates the subgroup. 
In our formalisation we prove that the set of reduced words corresponding to 
the elements of such a subset satisfy the properties N0, N1 and N2.\<close>
...
lex L2 word'. We use a modified version of it, lex L2 word, with an added condition, which 
...
text\<open>Given a monoidgetype set S, reduced representation of an element w in the group
generated by the set, is the unique reduced word in the span of S corresponding to
 w. Existence of the reduced representation is proved by the lemma above.\<close>
...
text\<open>Left subword returns the initial segment with a specified length, 
 of the word w\<close>
...
text\<open>r gen returns a well order on the groupgentype\<close>
definition r_gen
  where
...
text\<open>lex word is the lexicographical order on the lists, which naturally extends to
 words \<close>
definition lex_word
  where
...
text\<open>lex L2 word` defines an order on the elements of the group, as defined in 
Lyndon and Schupp. It compares two left tuples, by first comparing the minimal 
elements of the tuples in the lexicographic order, and if they are equal,
 it compares the maximal elements of tuples in terms of the same.\<close> 
...
lengths of  the reduced representation, and if they are equal, it compares them using the 
lex L2 word' order defined above.\<close>
...
text\<open>We right subword on the lines of left subword above.\<close>
...
text\<open>This proves that if two elements are not comparable in terms of the lex L2 
word order, their reduced representations are either the same or inverses of each
 other.\<close>
lemma 
  assumes "X \<in> (\<langle>S\<rangle> // (reln_tuple \<langle>S\<rangle>)) \<and> Y \<in> (\<langle>S\<rangle> // (reln_tuple \<langle>S\<rangle>))"
      and "length ((red_rep S) X) = length ((red_rep S) Y)"
...
text\<open>l concat appends a list of words\<close>
...
text\<open>This is analogous to cancel2 , and is defined on three words, cancel3 then 
proves the unique existence of such a word by constructive means.\<close>
...
text\<open>Follow lemma tells us that X' H A, spans the subgroup H. 
   The proof uses well ordering property to show that every subset contains a 
    least element.\<close>
...
text\<open>In the lemma n reduced cancel, we prove that if B is a subset of reduced words of elements 
...
text\<open>If H is a subgroup of the freegroup generated by A, the set of reduced words of 
X` H A, and their inverses, satisfy the N2 property, and thus as a consequence of 
the previous results N0 and N1, all the three N-properties. This is proved in the 
lemma N2 stated below.\<close>
